...
- [ ] Obtain explicit approval for one broad and one narrow synthetic Nova Lite request, then check current-artifact AI behavior. Auto-review blocked the first broad request before execution; no model call occurred during this deployment continuation. Each plan remains capped at two model calls, within the US$20 usable shared budget.
...
- [ ] Verify the original broad synthetic goal on the new artifact. The historical `50dad254...` run returned non-actionable `partial` after invalid finish arguments and the tool limit. The deployed replacement uses a top-three shortlist, trusted validation categories and at most two model calls, but its model behavior is unverified. Do not raise caps to conceal failures.
...
The constrained shared API/offline engineering demo is live and baseline-verified. A fictional AI workflow passed on an older revision and the superseded broad-goal test failed; neither result establishes current model behavior. Full AI MVP acceptance is incomplete pending explicit paid-test approval and actual checks. Teammate/browser acceptance remains open: each teammate must test their own organizer session, and the rendered interface still needs visual/keyboard checks. This is not readiness for real participant use. Login silence does not authorize credential extraction, password resets, public unauthenticated access or personal paid-provider substitution.
