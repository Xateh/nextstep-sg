...
- JavaScript tests: edits/reset discard local drafts and review; stale/malformed responses cannot restore them; exact review response preservation; expiry at rest and in-flight; safe timeout/network guidance without automatic retries; guarded downloads; simulated focus and duplicate-click handling. These do not prove rendered behavior.
...
- [ ] Visually verify browser workflow, keyboard focus, narrow layout and export. Automated in-app navigation to the local URL was blocked by the browser client; HTTP/state tests are not a substitute for rendered UI testing. On an allowed browser surface, rehearse offline: keyboard-only profile -> draft heading -> review -> enabled download; Edit profile retains fields but discards approval; Start over clears fields and restores offline/unchecked defaults; inspect 320px and desktop layout, long content, visible expiry and the actual downloaded file. Do not expose the local server or bypass the preview restriction.
