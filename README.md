...
**Constrained source deployed; current live AI acceptance pending:** the `5328bcb3...` package is `Active` / `Successful` on the existing IAM-authenticated endpoint. Signed routes and offline review/export guards passed on this exact package. Fresh local checks passed 72 Python tests and two interface-state tests. The planner supplies up to three constraint-filtered catalog records, accepts only shortlist IDs or applicable clarification, and permits at most two model calls including one repair. API routes and safeguards are unchanged. Auto-review blocked the first paid smoke request before execution; explicit approval for one broad and one narrow synthetic request is pending. No new model call has occurred. See [verification evidence](docs/VERIFICATION.md).

- Local offline workflow: implemented, including profile form, source cards, questions, review, export and trace.
- Bedrock workflow: current-package behavior is unverified. The earlier `f916c959...` revision passed one [fictional smoke request](examples/fictional-bedrock-request.json); the later, now superseded `50dad254...` revision returned a non-actionable broad-goal partial result at its four-model/six-tool limits. Neither historical result is acceptance of the newly deployed constrained workflow. No silent offline fallback exists.
...
The committed Bedrock example is the exact fictional input used for the historical prior-revision smoke test. It must not be treated as evidence that the current deployment or general planner is ready. The broad and narrow synthetic cases need explicit paid-case approval and a fresh budget check; do not rerun paid requests merely to reproduce historical evidence.
