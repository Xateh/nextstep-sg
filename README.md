...
**Deployed; two approved synthetic AI smoke cases passed:** the `5328bcb3...` package is `Active` / `Successful` on the existing IAM-authenticated endpoint. Broad and narrow requests each returned a Bedrock draft in one model call and passed exact-plan review/export and rejection guards. Total: two model calls, no repairs, retries or fallback. Local checks cover 72 Python and two interface-state tests. The planner supplies up to three constraint-filtered catalog records and permits at most two model calls per plan including one repair. These two cases do not establish general reliability, teammate access or rendered-interface readiness. See [verification evidence](docs/VERIFICATION.md).
